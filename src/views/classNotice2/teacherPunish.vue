    ...
    <div>
        ...
        <div>
            <div class="top">
                ...
                <div v-for="(item,index) in classList" :key="index" @click="chooseClass(index,item)"  :class="item.choose? 'className active':'className'">
                    {{item.GradeName}}{{item.ClassName}}
                    <img v-if="item.choose" src="@/assets/img/notice/check.png" alt="">
                </div>
            </div>
            <div class="mid">
...
export default {
    ...
    methods: {
        ...
        // 选择班级
        chooseClass (index, item) {
            item.choose = !item.choose
            this.$set(this.classList,index,item)
        },
        // 选择模板
        ...
        // 检测
        check () {
            if (this.classItem.length == 0) {
                this.$vux.toast.text('请选择通知班级')
                return false
            }
        ...
        AddNotice () {
            ...
            this.classList.forEach((item,index)=>{
                if (item.choose) {
                    ...
                    SchoolNames.push(item.SchoolName)
            ...
            this.$store.dispatch('media/upload').then(() => {
                ...
                this.$store.dispatch('notice2/AddNotice', params).then(
                    data => {
                        if (data.Code == 200) {
