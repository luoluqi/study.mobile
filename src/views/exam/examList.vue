...
export default {
    ...
    methods:{
      ...
      getSubjectList(){
        ...
        // 教师获取考试列表getExamListTeachBySubject
        getSubject(index,subject){
            // this.$refs.scroll.scrollHeight
          this.subject = subject
          this.activeClass = index
          this.$store.commit('exam/clearData')
          this.$store.dispatch('exam/getExamListTeachBySubject',subject)
        },
        loadMore(){
            if(!this.subject){
               this.$store.dispatch('exam/getExamListTeachBySubject',this.subjectData[0])
            }else{
                this.$store.dispatch('exam/getExamListTeachBySubject',this.subject)
            }
