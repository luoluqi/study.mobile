<template>
    <div class="pushParent">
        ...
        <div class="pushDetail">
            <p>{{studentData.CreateTime | formatDateTime('YYYY-MM-DD HH:mm:ss')}}</p>
            ...
            <div class="tangType">
                ...
                <span v-if="studentData.status == 4" class="chidao">早退</span>
            </div>
            <div class="tangTeacher">                           
                <span>点名教师：{{studentData.teachName}}</span>
            </div>
            <div class="tangTeacher">
                <span >班主任：{{studentData.headTeachName}}</span>
            </div>
        </div>
        <!-- <bottomNav></bottomNav> -->
    </div>
...
import {classRollsParent} from "@/api/pushApi"
export default {
    components:{
       headNav,
       bottomNav
