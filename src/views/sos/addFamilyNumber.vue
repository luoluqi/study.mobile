    ...
    <div>
        ...
        <group  label-width="6.5em" gutter="0.24rem" label-margin-right="2em" label-align="left">
            <div v-if="isOtner" class="rowStyle">
                <x-input type="text" title="家人关系：" :show-clear='false' text-align="left" :min=1 :max=10 v-model.trim="familyRelation" placeholder="请输入家人关系"></x-input>
            </div>
            <div class="rowStyle">
                <x-input type="text" :min=11 :max=11 :show-clear='false' title="亲情号码：" text-align="left" v-model.trim="familyNum" placeholder="请输入亲情号码"></x-input>
            </div>
        </group>
        <div @click="setOrUpdateFamilyPhones" class="remind">
            确认提交
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        setOrUpdateFamilyPhones () {
            ...
            var familyPhones = this.$store.state.sos.phoneList ? this.$store.state.sos.phoneList : []
            ...
            this.$store.dispatch('sos/SetOrUpdateFamilyPhones', params).then(
                ...
                this.$store.state.sos.phoneList.splice(this.$store.state.sos.phoneList.length-1,1)
            })
        },
        // 选择人
