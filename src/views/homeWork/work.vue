    ...
    <div id="Work">
        ...
        <div class="s_scetion" v-for="(item,index) in workList" :key="index">
            <div class="subWei">
                ...
                <div class="yiwei" @click="detailShow(item.Id)"><label>已读</label><span>{{item.readNumber}}</span><label>/{{item.Number}}</label></div>
            </div>
            <p>{{item.Content}}</p>
            <div class="zuoyeList">
                <ul >
                    <li @click="previewImg(item.Imgs,img)" v-for="(img,index) in item.Imgs" :key="index"><img class="fsf-img" :src="img | imgUrl" /></li>
                </ul>
            </div>
...
import {GetReadList} from '@/api/homeWorkApi'
export default {
     components:{
       headNav,
       bottomNav
