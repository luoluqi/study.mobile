    ...
    <div>
        ...
        <div class="main">
            <div class="top">
                ...
                <p v-for="(item,index) in formDetail.recordFieldList" :key="index" class="flowContent">
                    <span class="buyGood" v-if="item.FieldValue == 'buyGood'">{{item.FieldChinaName}}</span>
                    <span class="line" v-else-if="item.FieldValue == 'line'"></span>
                    <span class="label" v-else>{{item.FieldChinaName}} :　{{item.FieldValue}}</span>
                </p>
                <div class="detailImg">
...
export default {
    ...
    computed: {
       ...
       deptId () {
            return this.$store.state.oa.deptId
       }
    },
    mounted () {
        this.getFlow(true)
    ...
    methods: {
        ...
        // 获取流程信息
        getFlow (flag) { 
            this.$store.dispatch('oa/Item', {_id: this.dflowId}).then(res => { 
                if (res.Code == 200) { 
                    if (!flag) {
                        this.$store.state.oa.goIndexType = res.Data.FlowState
                        this.$router.go(-1)
                    }
        ...
        cancelFlow () {
            ...
            self.$vux.confirm.show({
                ...
                onConfirm () {
                    var params = {
                        _id: self.dflowId,
                        userId: self.$store.getters['user/teacherId']
                    }
                        ...
                        res => {
                            ...
                            if (result.Code == 200) {
                                that.$vux.alert.show({
                                    ...
                                    maskZIndex: 100,
                                    onHide () {
                                        that.$store.state.oa.goIndexType = 3
                                        that.$router.go(-1)
                                    }
                                })
                            } else {
