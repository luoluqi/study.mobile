<template>
    <div id="menu" class="menu-con">
        ...
        <div class="nav_title">
            ...
            <div class="nav_person" @click="showList">
              <div v-if="roleCode == 'Parent'">
                ...
                <img class="changest" src="@/assets/img/menu/arrow.png" />
              </div>
              <div v-else>
                <!-- <img class="per_img" :src="teacherImg" /> -->
                <span>
                  {{realName}}
        ...
        <div class="banner">
            ...
            <!-- <swiper :list="imgList" v-model="imgIndex" auto :show-desc-mask="false" height='2.3rem'></swiper> -->
              <Swiper>
                <Slide>
...
export default {
    ...
    created () {
        ...
        if (this.$store.state.user.isNoClass && this.$store.state.user.roleCode == 'Teacher') {
            this.$router.push('/menu/index-master')
            return
        } 
        this.roleCode = getCookie('roleCode')
        if (getCookie('roleCode') == 'Parent') {
          if(localStorage.getItem('itemData') == null) {
            this.getUrlNewList(getCookie('schoolId'), 1)
            this.getReadData(getCookie('studentId'), getCookie('classId'))
            this.getHeadList()
            this.getNewsList()
          } else {
            var itemData = JSON.parse(localStorage.getItem('itemData'))
            setCookie('classId', itemData.ClassId)
            setCookie('gradeId', itemData.GradeId)  
             setCookie('schoolId', itemData.SchoolId)
            // console.log(itemData)
            this.getUrlNewList(itemData.SchoolId, 1)
            this.getReadData(itemData.SelfId, itemData.ClassId)
            this.getHeadList()
            this.getNewsList()
          }
        } else {
          ...
          if (localStorage.getItem('itemDataTeacher') == null) {
            ...
            this.getReadData(getCookie('teacherId'), getCookie('classId'))
            this.getHeadList()
            this.getNewsList()
          } else {
            this.getUrlNewList(getCookie('schoolId'), 2)
            var itemDataTeacher = JSON.parse(localStorage.getItem('itemDataTeacher'))
             setCookie('classId', itemDataTeacher.ClassId)
            setCookie('gradeId', itemDataTeacher.GradeId)
            setCookie('schoolId', itemDataTeacher.SchoolId)

            this.getReadData(getCookie('teacherId'), itemDataTeacher.ClassId)
            this.getHeadList()
            this.getNewsList()
          }
          // console.log(getCookie('realName'))
          this.realName=decodeURI(getCookie('realName'))  // unescape
    ...
    methods: {
        ...
        getReadData (studentId,classId) {
          if (getCookie('roleCode') == 'Parent'){
            ...
            this.$store.dispatch('menu/GetReadDataSos',studentId).then(res => {
                this.sos = res.unReadCount
            })
          } else {
        ...
        getHeadList () {
          ...
          if(roleCode == 'Parent') {
            ...
            // setTimeout(() => {
              ...
              if (localStorage.getItem('itemDataTeacher') == null) {
                self.itemData = self.classData[0]
        ...
        getUrlNewList (schoolId, type) {
          var  key='urlList'+schoolId+type
            ...
            this.$store.dispatch('menu/GetUrlListV3',params).then(res => {
              ...
              if (result.Code == 200) {
                this.totalData = result.Data[0].featurepackExend
                localStorage.setItem(key,JSON.stringify(this.totalData))
              }
            })
